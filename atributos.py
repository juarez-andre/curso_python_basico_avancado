...
Atributos representam as características do objeto. Ou seja, pelos atributos nós conseguimos representar computacionalmente
...
# Atributos de instância: São atributosd declarados dentro do método construtor.
...
Caso queiramos demonstrar que determinado atributo deve ser tratado como privado, ou seja, que deve ser acessado/
utilizado somente dentro da própria classe onde está declarado, utiliza-se __ duplo underscore no início de seu nome.
...
class Teste:
    def __init__(cerveja, nome, idade):     # Não é legal, mas dá pra fazer
        cerveja.nome = nome
        cerveja.idade = idade
...
# Geralmente já inicializamos um valor, e este valopr é compartilhado entre todas as instâncias da classe.
...
# OBS: O atributo dinâmico serã exclusivo da instância que o criou.
...
print(f'Produto: {p1.nome}, Descrição: {p1.descricao}, Valor: {p1.valor}, Peso: {p1.peso}') # AttributeErrpr
