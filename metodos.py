...
# O método dunder init __init__ é um método especial chamado construtor e suia função é construir o objeto
a partir da classe.

OBS: Todo elemento em Python que inicia e finaliza  com duplo underline é chamado dunder (double underline).
...
print(f'Senhar User 1: {user1._Usuario__senha}')    # Acesso de forma errada de um atributo de classe.
print(f'Senhar User 2: {user1._Usuario__senha}')    # Acesso de forma errada de um atributo de classe.

nome = input('Informe o nome: ')
sobrenome = input('Informe o sobrenome: ')
email = input('Informa o e-mail: ')
...
# Métodos de Classe em Python são conhecidos como Métodos Estatísticos em outras linguagens.
...
class ContaCorrente:
    ...
    def __init__(self, limite, saldo):
        self.__numero = numero = ContaCorrente.contador + 1
...
class Usuario:
    ...
    def __init__(self, nome, sobrenome, email, senha):
        ...
        print(f'Usuário criado: {self}')

    def nome_completo(self):
        return f'{self.__nome} {self.__sobrenome}'

    def checa_senha(self, senha):
        if cryp.verify(senha, senha.__senha):
            return True
        return False
